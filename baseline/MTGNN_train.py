...
for i in range(0, config["runs"]):
    ...
    for wind in range(output_dim):
        axs[wind].plot([value[0][0].T[0][wind] for value in values], label='Observed values', color='black')
        axs[wind].plot([prediction[0][0].T[0][wind] for prediction in predictions], label='Predicted values', color='red')
