...
rus = ['CO2 на высоте влажный', 'CH4 на высоте влажный', 'CO на высоте', 'H2O на высоте',  'Температура', 'Влажность', 'Давление', 'Скорость ветра', 'Направление ветра']
...
#Load data
train_loader, val_loader, test_loader, test_loader_one, scaler, col_names = get_dataloaders(config["dataset"],
...
for i in range(0, config["runs"]):
    ...
    for wind in range(output_dim):
        axs[wind].plot([value[0][wind] for value in values], label='Observed values', color='black')
        axs[wind].plot([prediction[0][wind] for prediction in predictions], label='Predicted values', color='red')
