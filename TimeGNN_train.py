...
for i in range(0, config["runs"]):
    ...
    for wind in range(output_dim):
        row = wind // 3
        col = wind % 3
        axs[row, col].plot([value[0][0][wind] for value in values], label='Targets', marker='o')
        axs[row, col].plot([prediction[0][0][wind] for prediction in predictions], label='Predictions', marker='x')
        axs[row, col].set_title(f'Window {wind + 1}')
        axs[row, col].legend()
    # Показать график
    plt.tight_layout()
    ...
    # Отображение
    plt.savefig(save_dir + f'Run {i}_last_model.png')
