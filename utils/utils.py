...
def calculate_metrics(df):
    return {'mae': mean_absolute_error(df.value.tolist(), df.prediction.tolist()),
            ...
            'r2': r2_score(df.value.tolist(), df.prediction.tolist()),
            'mape': mean_absolute_percentage_error(df.value.tolist(), df.prediction.tolist()),
            'nrmse': (mean_squared_error(df.value.tolist(), df.prediction.tolist()) / mean_absolute_error(df.value.tolist(), df.prediction.tolist())) ** 0.5}
...
def get_experiment_number(output_dir):
    ...
    folders = [int(name.split("_")[1]) for name in os.listdir(output_dir) if
               os.path.isdir(output_dir + name) and "experiment" in name]
    folders.sort()
